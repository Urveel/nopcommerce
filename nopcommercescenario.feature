Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  Scenario: Verify that user can naviage to books category
  ...
  Scenario: Verify that user ca see'Name: A to Z' selection is present in 'Sort by' filter
    Given User is on Books Category page
    When  User is on Books category page
    And   Click on 'Sort by' filter
    And   Check that 'Name: A to Z' selection is present
    Then  'Name: A to Z' slection is present in 'Sort by' filter
